...
require(['jquery', 'scroll', 'template'], function ($, scroll, template) {
  ...
  $(function () {
    ...
    // 滚动条到底了加载更多
    scroll.clientScrollBar(function () {
      getHomeArticleList(page = page + 1);
    })
      ...
      success: function (result) {
        ...
        if (!result.code && result.data.length) {
          result.data.forEach(function (item) {
            var html = template('articleTemp', item);
